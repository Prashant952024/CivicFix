...
import { formatWorkerIssueDateTime } from "@/lib/worker-issues";
...
function getNotificationTypeBadge(type: NotificationRow["notification_type"]) {
  switch (type) {
    case "ASSIGNMENT":
      return <Badge variant="info" size="sm">Task Assignment</Badge>;
    case "STATUS_CHANGE":
      return <Badge variant="warning" size="sm">Status Update</Badge>;
    case "VERIFICATION":
      return <Badge variant="success" size="sm">Approval / Verification</Badge>;
    case "SYSTEM":
    default:
      return <Badge variant="outline" size="sm">System Notice</Badge>;
  }
}
    ...
    async function loadNotifications() {
      ...
      if (loadError) {
        if (import.meta.env.DEV) {
          console.error("Worker notifications load failed", loadError);
        }
  ...
  if (sessionProblem || error) {
    return (
      <EmptyState
        ...
        description={sessionProblem ?? error ?? "We could not load your field worker alerts right now."}
        action={
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description="Stay informed about new task assignments, manager reviews, and status updates."
        actions={
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center rounded-3xl border border-dashed border-border/80">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-sm mx-auto">
            {filterTab === "unread"
              ? "You are all caught up! No unread task alerts."
              : "You have not received any field notifications yet."}
          </p>
        </Card>
      ) : (
        <div className="space-y-6">
...
function NotificationItem({
  ...
  return (
    ...
    >
      ...
      >
        {notification.notification_type === "ASSIGNMENT" && <HardHat className="h-5 w-5" />}
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground flex items-center gap-1 ml-auto">
            ...
            {formatWorkerIssueDateTime(notification.created_at)}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          {notification.related_issue_id && (
            <Button size="sm" variant="outline" asChild className="h-7 text-xs px-2.5 rounded-lg border-teal-200 text-teal-800 hover:bg-teal-50">
              <Link to={`/app/worker/assigned-issues/${notification.related_issue_id}`}>
