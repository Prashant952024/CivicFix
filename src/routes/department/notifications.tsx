...
function getNotificationTypeBadge(type: NotificationRow["notification_type"]) {
  switch (type) {
    case "ASSIGNMENT":
      return <Badge variant="info" size="sm">Department Assignment</Badge>;
    case "STATUS_CHANGE":
      return <Badge variant="warning" size="sm">Task Update</Badge>;
    case "VERIFICATION":
      return <Badge variant="success" size="sm">Review & Approval</Badge>;
    case "SYSTEM":
    default:
      return <Badge variant="outline" size="sm">System Alert</Badge>;
  }
}
    ...
    async function loadNotifications() {
      ...
      if (loadError) {
        if (import.meta.env.DEV) {
          console.error("Department manager notifications load failed", loadError);
        }
        setError("Unable to load departmental notifications right now.");
        setNotifications([]);
        setLoading(false);
        return;
      }
  ...
  if (sessionProblem || error) {
    return (
      <EmptyState
        ...
        description={sessionProblem ?? error ?? "We could not load your department alerts right now."}
        action={
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        title="Department Alerts & Notifications"
        description="Stay informed on new municipal assignments, worker evidence submissions, and review requests."
        actions={
      ...
      {loading ? (
        ...
        <Card className="p-8 text-center rounded-3xl border border-dashed border-border/80">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-sm mx-auto">
            {filterTab === "unread"
              ? "You are all caught up! No unread departmental alerts."
              : "You have not received any departmental notifications yet."}
          </p>
        </Card>
      ) : (
        <div className="space-y-6">
...
function NotificationItem({
  ...
  return (
    ...
    >
      ...
      >
        {notification.notification_type === "ASSIGNMENT" && <Building2 className="h-5 w-5" />}
        {notification.notification_type === "STATUS_CHANGE" && <Wrench className="h-5 w-5" />}
        {notification.notification_type === "VERIFICATION" && <Sparkles className="h-5 w-5" />}
        {notification.notification_type !== "ASSIGNMENT" &&
          notification.notification_type !== "STATUS_CHANGE" &&
          notification.notification_type !== "VERIFICATION" && <Info className="h-5 w-5" />}
      </div>
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground flex items-center gap-1 ml-auto">
            ...
            {formatCitizenIssueDateTime(notification.created_at)}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          {notification.related_issue_id && (
            <Button size="sm" variant="outline" asChild className="h-7 text-xs px-2.5 rounded-lg border-teal-200 text-teal-800 hover:bg-teal-50">
              <Link to={`/app/manager/tasks/${notification.related_issue_id}`}>
